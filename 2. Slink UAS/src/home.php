...
if (isset($_POST["submit_search"])) {
    ...
    $search_users_id = queryGetData("SELECT id FROM users WHERE username LIKE '%{$_POST['keyword']}%'");
}
        ...
        <section class="container m-auto mt-3 p-3 w-50">
            ...
            <form action="" method="POST">
                <div class="input-group mb-4 mt-3 row mx-2">
                    <div class="col-1"></div>
                    <div class="col-8">
                        <input type="text" list="list_user" id="keyword" name="keyword" placeholder="Cari Posts/Users" class="form-control mb-2" />
            ...
            <div class="row" id="new_posts">
                ...
                <!-- Ketika Users Ada -->
                <?php if (isset($search_users_id)) : ?>
                    <?php foreach ($search_users_id as $sui) : ?>
                        <?php $user = getProfile($sui['id'])[0]; ?>
                        <?php $followCondition = (checkFollows($user['id'], $user_id)) ? "<button type='button' class='col-3 btn btn-outline-danger follow_button' data-id='" . $user['id'] . "'>Unfollow</button>" : "<button type='button' class='col-3 btn btn-primary follow_button' data-id='" . $user['id'] . "'>Follow</button>"; ?>
                        <?php $fotoCondition = ($user['foto'] == null) ? "<img class='rounded-circle shadow-1-strong me-3' src='../Foto/user.png' alt='avatar' width='50' height='50' />" : "<img class='rounded-circle shadow-1-strong me-3' src='../Foto/" . $user['foto'] . "' alt='avatar' width='50' height='50' />"; ?>
                        <div class="item" style="width: 600px ; margin-left: 75px">
                            <div class="card mb-5" style="width: 550px ;">
                                <div class="card-body d-flex">
                                    <a target="_blank" href="./user.php?user_id=<?= $user['id'] ?>" class="col-9 row text-decoration-none text-reset">
                                        <div class="col-2">
                                            <?= $fotoCondition; ?>
                                        </div>
                                        <h6 class="col-4"><?= $user['username']; ?></h6>
                                        <div class="col-2">
                                            <h7 class="text-center bi bi-heart-fill text-danger"></h7>
                                            <span class="mx-1"><?= $user['count_likes']; ?></span>
                                        </div>
                                        <div class="col-2 ">
                                            <h7 class="text-center bi bi-card-heading text-primary"></h7>
                                            <span class="mx-1"><?= $user['count_posts']; ?></span>
                                        </div>
                                    </a>
                                    <?= $followCondition; ?>
                                </div>
                ...
                <?php foreach ($posts as $post) : ?>
                    ...
                    <?php $likeCondition = (checkPostliked($user_id, $post['id'])) ? 'bi bi-heart-fill text-danger fs-5 mx-1 like_button' : 'bi bi-heart text-danger fs-5 mx-1 like_button' ?>
