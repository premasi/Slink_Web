        ...
        <section class="container m-auto mt-3 p-3 w-50">
            ...
            <form action="" method="POST">
                <div class="input-group mb-4 mt-3 row mx-2">
                    <div class="col-1"></div>
                    <div class="col-8">
                        <input type="text" class="form-control" placeholder="Cari Posts..." id="keyword"
                            name="keyword" />
                    </div>
                    <div class="col-2">
                        <button type="submit" class="btn btn-outline-success col-12" type="button"
            ...
            <div class="row" id="new_posts">
                ...
                <div class="media border p-3 mb-3 shadow w-75 m-auto">
                    <div class="media-body">
                        <h2><a style="text-decoration: none;color:black"
                                href="posts.php?p_id=<?= $post['id']; ?>"><?= $post['judul']; ?></a>
                            <h2>
                                <div class="d-flex">
                                    <h5 class=""><span style="color: #45625D;">by</span><?= " " . $post["username"]; ?>
                                    </h5>
                                ...
                                <i <?php if (checkPostLiked($user_id, $post['id'])) : ?>
                                    class="bi bi-heart-fill text-danger fs-5 mx-1 like_button" <?php else : ?>
                                    class="bi bi-heart fs-5 mx-1 like_button" <?php endif ?>
                                    data-id="<?= $post["id"] ?>"></i>
                                <i class="bi bi-chat fs-5 mx-1 comment_button" data-bs-toggle="modal"
                                    data-bs-target="#modal_form" data-id="<?= $post['id'] ?>"></i>
                                <i <?php if (checkPostBookmarked($user_id, $post['id'])) : ?>
                                    class="bi bi-bookmark-fill text-primary fs-5 mx-1 bookmark_button" <?php else : ?>
                                    class="bi bi-bookmark fs-5 mx-1 bookmark_button" <?php endif ?>
                                    data-id="<?= $post["id"] ?>"></i>
            ...
            <div class="text-center">
                <button class=" btn btn-outline-success btn-lg me-auto rounded-pill" id="button_seeMore" type="button"
                    <?php if (count($posts) < $_SESSION['limit']) : ?> style="display: none;" <?php endif;  ?>>See
                    More</button>
                <button class=" btn btn-outline-danger btn-lg me-auto rounded-pill" id="button_seeLess" type="button"
                    <?php if ($_SESSION['limit'] == 5) : ?> style="display: none;" <?php endif;  ?>>See Less</button>
            </div>
        </section>
                        ...
                        <div class="row mb-2 mx-1">
                            <div class="col-8">
                                <textarea class="form-control form-control-sm" id="comment" name="comment"
                                    placeholder="Tulis comment..." required></textarea>
                            </div>
                            <div class="col-3">
                                <button type="submit" class="btn btn-outline-primary"
                                    name="button_submit_comment">Comment</button>
                            </div>
                        </div>
                    </form>
                </div>
